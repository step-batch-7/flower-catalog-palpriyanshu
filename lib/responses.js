...
const previousComment = readFileSync(config.DATA_STORE, 'utf8');
...
const savePost = function(contentType, body) {
  ...
  writeFileSync(config.DATA_STORE, JSON.stringify(comments.comments, null, 2));
};
...
const updateComments = function(comments) {
  let guestBook = readFileSync(
    `${STATIC_FOLDER}/template/guestBook.html`,
    'utf8'
  );
